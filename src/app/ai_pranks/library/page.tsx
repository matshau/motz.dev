...
export default function Library() {
  // Fake: show first 2 pranks as "your" generated pranks
  const myPranks = communityPranks.slice(0, 2);
  ...
  return (
    <div className="min-h-screen bg-black font-[family-name:var(--font-cerebri-sans)] text-white">
      ...
      <section className="mt-6 px-5">
        <h2 className="mb-4 px-3 text-base font-bold tracking-tighter text-white/60">My Library</h2>
        <div className="grid grid-cols-2 gap-3">
          {myPranks.map((prank, i) => (
            <div key={i} className="relative overflow-hidden rounded-2xl" style={{ aspectRatio: "3/4" }}>
              ...
              <div className="absolute bottom-0 left-0 right-0 z-10 p-3">
                <p className="text-sm font-bold tracking-tighter">{prank.title}</p>
                <p className="mt-1 text-[10px] text-white/50">Just now</p>
              </div>
            </div>
          ))}
        </div>
